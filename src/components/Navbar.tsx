...
const Navbar = () => {
  ...
  return (
    ...
    >
      <nav
        className={`pointer-events-auto transition-all duration-700 ease-in-out px-2 py-1.5 rounded-full border border-green-500/10 backdrop-blur-md shadow-2xl flex items-center justify-between gap-4 sm:gap-8 ${isScrolled
          ...
          {isMobileMenuOpen && (
            <>
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 bg-black/40 backdrop-blur-sm -z-10 md:hidden h-screen w-screen"
                onClick={() => setIsMobileMenuOpen(false)}
              />
              <motion.div
                ...
                className="absolute top-16 left-0 right-0 mx-auto w-[90vw] max-w-sm bg-black/60 backdrop-blur-3xl border border-green-500/10 rounded-2xl p-3 shadow-2xl md:hidden"
              >
